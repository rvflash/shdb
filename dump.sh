...
while getopts ":h::u::p:d::t::b:l:f:w:DTv" FLAG; do
    case "${FLAG}" in
        ...
        b) if [ "${OPTARG:0:1}" = "/" ]; then BACKUP_ROOT="$OPTARG"; elif [ "$OPTARG" != "" ]; then BACKUP_ROOT="$SCRIPT_ROOT$OPTARG"; fi ;;
...
for DB_NAME in ${DB_NAMES}; do
    ...
    for TABLE_NAME in ${TABLE_NAMES}; do
        ...
        if [ "$WITH_TMP_TABLE" -eq 0 ] && ([[ "$TABLE_NAME" == "_"* ]] || [[ "$TABLE_NAME" == *"_TMP"* ]]); then
            if [ "$VERBOSE" != "" ]; then
        ...
        # One folder by table name (remove redundant slash) and clean workspace
