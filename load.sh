...
ERR_FILE="${TMP_DIR}${CURDATE}_shbd_load.err"
TABLE_NAMES=""
OPTIONS=""
VERBOSE=""
DRY_RUN=0;
...
while getopts "b:d::h::u::p::t:l:DBrv" FLAG; do
    case "${FLAG}" in
        b) if [ "${OPTARG: -1}" = "/" ]; then BACKUP_ROOT="$OPTARG"; else BACKUP_ROOT="$SCRIPT_ROOT$OPTARG"; fi ;;
        d) DB_NAMES="$OPTARG" ;;
